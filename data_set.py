...
class MVClaDataset_k(Dataset):
    def __init__(self,
        ...
        # self.label = self.label[:int(len(self.label) / 2)]
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'train':
            ...
            if np.random.rand() < 0.5:
                ...
                image = image[:, :, cut_size:-cut_size, cut_size:-cut_size]
                label = label[:, :, cut_size:-cut_size, cut_size:-cut_size]
            else:
                # 外圈填充0,shape: (c, h, w, d),h和w随机填充
                padd_size = np.random.randint(1, 16)
                image = np.pad(image, ((0, 0), (0, 0), (padd_size, padd_size), (padd_size, padd_size)), 'constant')
                label = np.pad(label, ((0, 0), (0, 0), (padd_size, padd_size), (padd_size, padd_size)), 'constant')
            #
            if np.random.rand() < 0.5:
                # 随机平移-5到5
        ...
        label = label[0:1]
        label = F.one_hot(label.long(), num_classes=3).permute(0, 4, 1, 2, 3).squeeze(0).float()
        label_ = label
        ...
        # image_pil = image.numpy()[0, 100, :, ::-1]
        # image_pil = image_pil * 255
        # image_pil = image_pil.astype(np.uint8)
        # image_pil = Image.fromarray(image_pil)
        # image_pil.save('./temp0.png')
        # image_pil = image.numpy()[0, 100, :, :]
        # image_pil = image_pil*255
        # image_pil = image_pil.astype(np.uint8)
        # image_pil = Image.fromarray(image_pil)
        # image_pil.save('./temp1.png')
        image = (image - 0.5) / 0.5
        label[label > 0.5] = 1
        label[label <= 0.5] = 0
        # # sobel提取边界
        ...
        #
        if self.split == 'test':
            return image, label_, self.data[idx], torch.tensor((d, w, h), dtype=torch.int32)
        else:
            return image, label, cls


class MVClaDataset_all(Dataset):
    def __init__(self,
                 base_dir,
                 split='train',
                 fold=0,
    ...
    def __getitem__(self, idx):
        ...
        image = F.interpolate(torch.unsqueeze(image, 0), size=(16, 224, 224), mode='trilinear', align_corners=True).squeeze(0)
        ...
        return image, (c, d, w, h), self.data[idx]
...
if __name__ == '__main__':
    ...
    train_data = MVClaDataset_k('../data/', 'train')
