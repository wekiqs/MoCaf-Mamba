...
def sigmoid_weighted_loss(output, target, num_cls=5):
    target = target.float()
    B, _, H, W, Z = output.size()
    total_pixel_count = torch.sum(target, (1, 2, 3, 4)) + 1e-6
    output = torch.clamp(output, min=0.005, max=1)
    for i in range(num_cls):
        probs_i = output[:, i, :, :, :]
        target_i = target[:, i, :, :, :]
        class_count = torch.sum(target_i, (1, 2, 3))
        weighted_val = 1.0 - (class_count / total_pixel_count)
        weight_map = weighted_val.view(B, 1, 1, 1).expand(B, H, W, Z)
        weighted_bce = -1 * weight_map * (target_i * torch.log(probs_i) + (1.0 - target_i) * torch.log(1.0 - probs_i))

        if i == 0:
            total_loss = torch.mean(weighted_bce)
        else:
            total_loss += torch.mean(weighted_bce)
